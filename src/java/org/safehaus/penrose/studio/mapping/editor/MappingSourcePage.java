...
import org.safehaus.penrose.studio.mapping.editor.FieldSelectionDialog;
 ...
 */
public class MappingSourcePage extends FormPage implements ModifyListener {
    ...
    public MappingSourcePage(MappingEditor editor) {
    ...
    public void createFormContent(IManagedForm managedForm) {
        ...
        section.setText("Relationships");
        section.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

        Composite relationshipSection = createRelationshipsSection(section);
        section.setClient(relationshipSection);
    ...
    public Composite createRelationshipsSection(Composite parent) {
        ...
        relationshipTable = toolkit.createTable(composite, SWT.BORDER | SWT.MULTI | SWT.FULL_SELECTION);
        GridData gd = new GridData(GridData.FILL_BOTH);
        gd.heightHint = 100;
        relationshipTable.setLayoutData(gd);
        relationshipTable.setLayout(new FillLayout());

        refreshRelationships();

        relationshipTable.addMouseListener(new MouseAdapter() {
            public void mouseDoubleClick(MouseEvent event) {
                if (relationshipTable.getSelectionCount() == 0) return;

                TableItem item = relationshipTable.getSelection()[0];
                Relationship relationship = (Relationship)item.getData();

                RelationshipDialog dialog = new RelationshipDialog(editor.getParent().getShell(), SWT.NONE);
                dialog.setRelationship(relationship);
                dialog.setText("Edit relationship...");

                Partition partition = editor.getPartition();
                Collection dataSources = partition.getEffectiveSourceMappings(entry);
        ...
        removeButton.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent event) {
                ...
                for (int i=0; i<items.length; i++) {
                    ...
                    entry.removeRelationship(relationship);
                    items[i].dispose();
                }
                checkDirty();
            }
