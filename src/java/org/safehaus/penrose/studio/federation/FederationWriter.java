...
public class FederationWriter {
    ...
    public Element createElement(FederationConfig federationConfig)  {
        ...
        for (RepositoryConfig repositoryConfig : federationConfig.getRepositoryConfigs()) {
            element.add(createElement(repositoryConfig));
        }

        return element;
    }

    public Element createElement(RepositoryConfig repositoryConfig) {
        Element element = new DefaultElement("repository");
        element.addAttribute("name", repositoryConfig.getName());
        element.addAttribute("type", repositoryConfig.getType());

        for (String name : repositoryConfig.getParameterNames()) {
            String value = repositoryConfig.getParameter(name);
